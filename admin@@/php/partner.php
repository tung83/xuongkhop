...
function partner_detail($db)
{
	...
	if(isset($_POST["addNew"])) {
            $insert = array(
                ...
                $recent = $db->insert($table,$insert);
                if(common::file_check($_FILES['file'])){
                    WideImage::load('file')->resize(400,320, 'fill')->saveToFile(myPath.$file);
                    $db->where('id',$recent);
                    $db->update($table,array('img'=>$file));
                }
                header("location:".$_SERVER['REQUEST_URI'],true); 
    ...
    $str.=$form->search_area($db,$act,'partner_detail_cate',$_GET['hint'],0);
